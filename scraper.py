...
def extract_question_and_answers(soup):
    question_card = soup.findAll('div', class_='card exam-question-card')[0]
    question_number = question_card.find('div', class_='card-header').text.strip().split()[1]
    question_topic = question_card.find('span', class_='question-title-topic').text.strip()
    question_text = question_card.find('div', class_='card-body').p.text.strip()
    answer_choices = {}
    for choice in question_card.find_all('li', class_='multi-choice-item'):
